...
function render(){if(!record)return;$('#rename').textContent=record.title;const f=record.forms[kind],v=f.values;$$('[data-field]').forEach(el=>{const k=el.dataset.field;if(document.activeElement!==el&&!dirty.has(k))el.value=v[k]??'';if(el.tagName==='TEXTAREA'){el.style.height='auto';el.style.height=Math.max(45,el.scrollHeight)+'px'}const wrap=el.closest('[data-wrap]'),meta=f.meta[k];wrap?.classList.toggle('agent',meta?.owner==='agent');wrap?.classList.toggle('user',meta?.owner==='user');wrap?.classList.toggle('estimated',!!meta?.source?.estimated&&v[k]!==''&&v[k]!=null);const s=wrap?.querySelector('[data-source]');if(s)s.hidden=!meta?.source||!Object.keys(meta.source).length});$$('[data-date-display]').forEach(el=>{const parts=(v[el.dataset.dateDisplay]||'').split('-');el.textContent=parts.length===3?String(+parts[el.dataset.part==='month'?1:2]):''});$$('[data-derived]').forEach(el=>el.textContent=v[el.dataset.derived]??'');if(!dirty.has('reasons'))$$('[data-reason]').forEach(el=>el.checked=(v.reasons||[]).includes(el.dataset.reason));if(!dirty.has('plane'))$$('[data-plane]').forEach(el=>el.checked=v.plane===el.value);for(const k of ['start','end'])if(document.activeElement!==$('#report-'+k)&&!dirty.has(k))$('#report-'+k).value=v[k]||'';
 ...
 $('#checks').innerHTML=(pipeline.failed_files?.length?`<div class="warn">未识别成功：${pipeline.failed_files.map(esc).join('、')}。成功材料已继续填写。</div>`:'')+[...(pipeline.errors||[]),...check.warnings].map(w=>`<div class="warn">${esc(w)}</div>`).join('')+(check.missing.length&&generated?`<span>无依据的空白项：${check.missing.slice(0,6).map(x=>esc(x.label)).join('、')}。可直接在表格中补充。</span>`:'');
  if(!dirty.has('has_fee'))$('#has-fee').value=v.has_fee||'';
